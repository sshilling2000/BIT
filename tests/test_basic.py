...
samples = samplers.fit(samplers.log_prob, initial_guess=[1.0, 1.0], args=(x, y, yerr))
...
#plot the results
fig = corner.corner(samples, labels=['m_fit','c_fit'], truths=[m_true, c_true])

#plot the data with our fit values over the true value
fig, axes = plt.subplots(1, figsize=(7, 7))
axes.errorbar(x=x,y=y,yerr=yerr,fmt='.',label='Data with noise from the linear model')
axes.plot(x,line,label='Linear model: \nm_true = {}, c_true = {}'.format(m_true,c_true))
axes.plot(x,model([m_fit,c_fit],x),c='red',label='Fitted model: \nm_fit = {}, c_fit = {}'.format(m_fit,c_fit))
plt.legend()
plt.show()